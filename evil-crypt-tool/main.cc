							...
							const char *cnt_filename)
{
	...
	if (key_header.v1.payload_size < 32) {
		std::cerr << "Указаный ключ имеет недостаточную длину для данного алгоритма шифрования" << std::endl;
		exit(1);
	}
							...
							const char *dst_filename)
{
	constexpr uint32_t BLOCK_SIZE = 8;
	uint32_t block_count;
	std::cout << "Расшифрование файла" << cnt_filename << " алгоритмом ГОСТ 34.12-64 в режиме CBC\n";
	std::ifstream key_file { key_filename, std::ios::binary };
	if (not key_file) {
		std::cerr << "Не удалось открыть файл ключа" << std::endl;
		exit(1);
